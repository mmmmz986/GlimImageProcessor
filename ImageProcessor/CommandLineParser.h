...
struct ProgramOptions {
	...
	std::string pipeline;    ///< --pipeline / -p (커맨드 추가)
    std::vector<std::string> filterFromPipeline;
    std::vector<std::string> valueFromPipeline;
...
public:
    ...
    /// 파이프라인 파서
    static void PipelineParse(ProgramOptions& options);
};
